#!/usr/bin/env python3
"""
Image Virtual Metrology (VM) Regression - Local Model Support
==============================================================
支持从本地 pytorch_model.bin 加载预训练权重，无需联网。
"""
...
class EfficientNetV2SRegressor(nn.Module):
    ...
    def load_local_ckpt(self, path, device='cpu'):
        """从本地 .bin 文件加载权重（支持 HuggingFace / timm 格式）"""
        ...
        print(f"Loaded local checkpoint from {path}")
        if missing:
...
def run_cross_validation(args):
    ...
    if args.local_ckpt:
        print(f"Using local checkpoint: {args.local_ckpt}")
    ...
    for fold, (train_gidx, val_gidx) in enumerate(kf.split(unique_groups)):
        ...
        # 创建模型：优先使用本地 checkpoint
        pretrained = (args.local_ckpt is None)
        model = EfficientNetV2SRegressor(pretrained=pretrained, dropout=args.dropout).to(device)
        if args.local_ckpt:
            model.load_local_ckpt(args.local_ckpt, device)
...
def run_inference(args):
    ...
    if args.local_ckpt:
        ...
        raise ValueError("For inference, provide either --local_ckpt or --model_path")
    
    if args.model_path and os.path.exists(args.model_path):
        # 如果同时提供了 model_path（微调后的权重），优先加载
...
def parse_args():
    parser = argparse.ArgumentParser(description='Image VM Regression with EfficientNetV2-S (Local Model Support)')
    parser.add_argument('--mode', type=str, required=True, choices=['train', 'train_all', 'infer'])
    parser.add_argument('--csv', type=str, default='data.csv')
    parser.add_argument('--image_root', type=str, default='')
    parser.add_argument('--output_dir', type=str, default='./output')
    parser.add_argument('--model_path', type=str, default=None, help='Fine-tuned model checkpoint (.pth)')
    parser.add_argument('--local_ckpt', type=str, default=None, help='Local pretrained backbone weights (.bin or .pth)')
...
# ============================================
    main(
        mode='train',
        csv='data/train.csv',              # CSV 路径：filename, tilt, group
        image_root='./images',             # 图片根目录
        output_dir='./exp_cv',             # 输出目录
        model_path=None,                   # 不从 checkpoint 恢复（交叉验证从头开始）
        local_ckpt='./pretrained/efficientnetv2_s/pytorch_model.bin',  # 本地预训练权重；None 则自动下载 ImageNet 预训练
        
        img_size=480,                      # 输入尺寸（EfficientNetV2-S 原生支持 384-480）
        ...
        lr=5e-4,                           # Head 学习率（Backbone 自动 ×0.1，除非 uniform_lr=True）
        weight_decay=1e-4,                 # 权重衰减
        rank_weight=0.5,                   # 组内 Rank Loss 权重（0=纯 MSE，1=与 MSE 同等权重）
        patience=15,                       # 早停耐心值（MAE 不下降则停止）
        
        n_splits=5,                        # KFold 折数
        fold=None,                         # 只跑某一折（0~4），None 则跑全部 5 折
        seed=42,                           # 随机种子
        
        input_image=None,                  # 训练模式不需要单图路径
        augment=True,                      # 启用图像增强（RandomFlip/Rotation/ColorJitter）
        uniform_lr=False,                  # False=分层学习率（Backbone lr×0.1）；True=所有层统一 lr
        lr_scheduler='cosine',             # 'cosine'=CosineAnnealingLR；'cosine_wr'=Warm Restarts
        scheduler_t0=10,                   # cosine_wr 的 T_0 周期（仅 lr_scheduler='cosine_wr' 时有效）
        
        group_sampler=True,                # True=GroupBatchSampler（同 batch 同组，Rank Loss 有效）
    )
    ...
    main(
        ...
        model_path='./exp_cv/fold_0_best.pth',  # 从最佳 fold 的 checkpoint 恢复；None 则从头训练
        local_ckpt=None,                   # 若已提供 model_path（含完整权重），local_ckpt 可设为 None
        ...
        epochs=80,                         # 全量训练可适当增加 epoch（无验证集早停限制）
        lr=5e-4,
        weight_decay=1e-4,
        rank_weight=0.5,
        patience=15,                       # train_all 模式未启用早停，保留参数统一
        
        n_splits=5,                        # 全量训练不拆折，保留参数占位
        fold=None,
        seed=42,
        ...
        group_sampler=True,                # 全量训练同样建议用 GroupBatchSampler
    )


    # ============================================
    # 示例 3：批量推理 (mode='infer')
    # ============================================
    # 推理模式下，训练相关参数（epochs/lr/rank_weight/patience 等）不会被使用，
    # 但为保持"所有参数显式"的要求，仍完整写出。
    main(
        mode='infer',
        csv='data/test.csv',               # 批量推理 CSV（含 filename, tilt, group）
        image_root='./images',
        output_dir='./exp_final',          # 推理结果 CSV 保存目录
        model_path='./exp_final/final_model.pth',  # 微调后的完整模型权重（优先加载）
        local_ckpt='./pretrained/efficientnetv2_s/pytorch_model.bin',  # 本地预训练权重（若 model_path 已包含完整权重则可省略）
        
        img_size=480,                      # 必须与训练时完全一致
    ...
    main(
        ...
        image_root='',                     # 单图路径已完整，无需根目录
        output_dir='./exp_final',
        model_path='./exp_final/final_model.pth',  # 微调后的完整模型
        local_ckpt=None,                   # 若 model_path 已包含完整权重，无需本地预训练
        
        img_size=480,
        batch_size=1,                    # 单图推理 batch_size=1
        num_workers=0,                   # 单图推理不需要多进程
        dropout=0.3,
        ...
        input_image='./test_images/wafer_001.jpg',  # 单张图片完整路径
        augment=False,
        uniform_lr=False,
        lr_scheduler='cosine',
        scheduler_t0=10,
    ...
    # ============================================
    # 示例 5：五折 CV（使用本地预训练权重，无网络环境）
    # ============================================
    main(
        mode='train',
        csv='data/train.csv',
        image_root='./images',
        output_dir='./exp_cv_local',
        model_path=None,
        local_ckpt='./pretrained/efficientnetv2_s/pytorch_model.bin',  # 必须提供本地权重文件
        
        img_size=480,
        batch_size=16,                     # 本地预训练时 batch 可稍小（显存占用）
        num_workers=4,
        dropout=0.3,
    ...
    # ============================================
    # 示例 6：全量训练（从本地预训练开始，不依赖网络）
    # ============================================
    main(
        mode='train_all',
        csv='data/train.csv',
        image_root='./images',
        output_dir='./exp_final_local',
        model_path=None,                   # 不从微调 checkpoint 恢复
        local_ckpt='./pretrained/efficientnetv2_s/pytorch_model.bin',  # 仅加载本地预训练 backbone
    ...
    # ============================================
    # 示例 7：推理（仅加载本地预训练，无微调权重时）
    # ============================================
    # 注意：这种情况 head 是随机初始化的，预测结果无意义，
    # 仅用于演示 local_ckpt 独立加载的能力。
    main(
        mode='infer',
        csv='data/test.csv',
        image_root='./images',
        output_dir='./exp_debug',
        model_path=None,                   # 无微调权重
        local_ckpt='./pretrained/efficientnetv2_s/pytorch_model.bin',  # 仅加载 backbone 预训练
